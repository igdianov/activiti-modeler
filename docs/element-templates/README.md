...
Alternatively, they can be stored in a `.activiti/element-templates` directory that resides, relative to the currently opened diagram, anywhere in the diagrams path hierarchy.
...
For local template discovery, create a `.activiti/element-templates` folder relative in the directory
...
As of Activiti Modeler `v1.11.0` we support special scoped bindings that allow you to configure nested elements, such as [connectors](https://docs.camunda.org/manual/7.7/user-guide/process-engine/connectors/#use-connectors).
